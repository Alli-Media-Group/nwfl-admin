  ...
  return (
    <div className="space-y-6">
      {!dismissed && pendingAlerts.length ? (
        <div className="shell-panel flex flex-wrap items-center justify-between gap-4 rounded-[22px] px-5 py-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.24em] text-amber-300">Pending Alerts</p>
            <p className="mt-1 text-sm text-[var(--color-off-white)]">
        ...
        {cards.map(({ icon: Icon, label, tone, value }) => (
          <Card className="p-5" key={label}>
            <div className="mb-8 flex items-start justify-between">
              <div className={`rounded-2xl p-3 ${tone === 'warning' ? 'bg-amber-500/15 text-amber-300' : 'bg-white/10 text-white'}`}>
                <Icon size={20} />
              </div>
              {tone === 'warning' && value > 0 ? <Badge value="PENDING" /> : null}
            </div>
            <p className="text-sm text-[var(--color-muted)]">{label}</p>
            <h3 className="mt-2 text-4xl text-[var(--color-off-white)]">
              <CountUp value={value} />
            </h3>
          </Card>
        ))}
      ...
      <Card className="overflow-hidden p-0">
        <div className="flex items-center justify-between border-b border-[var(--color-border)] px-5 py-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.24em] text-[var(--color-highlight)]">Recent Matches</p>
            <h3 className="mt-1 text-2xl text-[var(--color-off-white)]">Latest results and fixtures</h3>
          </div>
          <button className="btn-outline" onClick={() => navigate('/matches')} type="button">
            Open Desk
          </button>
            ...
            <tbody>
              {recentMatches.map((match) => (
                <tr key={match.id}>
                  <td>{match.date}</td>
                  <td>
                    <div className="font-medium text-[var(--color-off-white)]">
      ...
      <div className="data-grid data-grid-2">
        <Card className="p-5">
          <p className="text-xs font-semibold uppercase tracking-[0.24em] text-[var(--color-highlight)]">Desk Tempo</p>
          <h3 className="mt-2 text-2xl text-[var(--color-off-white)]">Tonight&apos;s live operations</h3>
          <div className="mt-5 space-y-4">
            {matches
              .filter((match) => match.status === 'LIVE' || match.status === 'UPCOMING')
              .slice(0, 4)
              .map((match) => (
                <div className="flex items-center justify-between rounded-2xl border border-[var(--color-border)] bg-black/20 px-4 py-3" key={match.id}>
                  <div>
                    <p className="font-medium text-[var(--color-off-white)]">
                      {match.home_team.short_name} vs {match.away_team.short_name}
                    </p>
                    <p className="mt-1 text-sm text-[var(--color-muted)]">{match.venue || 'Venue pending'}</p>
                  </div>
                  <div className="text-right">
                    <div className="inline-flex items-center gap-2 text-sm text-[var(--color-off-white)]">
